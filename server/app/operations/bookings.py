...
from app.models.bookings import (
    AirbnbBooking as AirbnbBookingDBModel,
    BookingcomBooking as BookingcomBookingDBModel
)
from app.schemas.bookings import AirbnbBooking, BookingcomBooking
...
def get_bookingcom_bookings(db_session: Session):
    bookings = jsonable_encoder((db_session.scalars(select(BookingcomBookingDBModel))).all())
    if not bookings:
        raise HTTPException(status_code=404, detail="No bookings found")
    return Ok200(records=bookings)


def post_bookingcom_bookings(db_session: Session, bookings: List[BookingcomBooking]):
    booking_models = [BookingcomBookingDBModel(**booking.model_dump()) for booking in bookings]
    try:
        db_session.add_all(booking_models)
        db_session.commit()
        return Created201(message=f"{len(bookings)} records created", records=bookings)
